...
- `convert` — convert a PDF into `.md` and `.json` using docling. Uses `TableFormerMode.ACCURATE` for table extraction, `ImageRefMode.PLACEHOLDER` for images. Options: `--ocr` (enable OCR, off by default), `--pages 1-50` (page range), `--force` (overwrite existing outputs), `--heading-strategy {bookmarks,numbering,none}` (control heading hierarchy post-processing; default `bookmarks`), `--no-drop-empty-bookmarks` (keep redundant index entries and disable structural-leaf heuristic), `--no-fuzzy-match` (disable case-insensitive bookmark-to-content matching). After postprocessing, bookmark casing is restored so headings use the clean bookmark titles rather than docling's extracted text. Prints OCR hint when pages have no extractable text.
